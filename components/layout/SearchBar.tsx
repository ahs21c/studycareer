...
export default function SearchBar({
  ...
  const [results, setResults] = useState<Array<{ slug: string; name: string; filings: number; hasPerm: boolean }>>([])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const matches = Object.values(COMPANIES)
        ...
        .slice(0, 8)
        .map(c => ({ slug: c.slug, name: c.employer_name, filings: c.lca_total_2yr, hasPerm: c.has_perm }))
      setResults(matches)
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', width: '100%' }}>
      <div style={{ position: 'relative' }}>
        {/* Search icon */}
        <svg
          style={{ position: 'absolute', left: large ? 12 : 9, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af', pointerEvents: 'none' }}
          ...
          {results.map((r, i) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, color: '#9ca3af' }}>{r.filings.toLocaleString()}</span>
              </div>
              {r.hasPerm && (
