...
const SECTOR_LABELS: Record<string, string> = {
  ...
  ARTS_ENTERTAINMENT: "Arts & Entertainment", AUTOMOTIVE_MFG: "Automotive", BANKING: "Banking & Finance",
  CHEMICAL_MFG: "Chemical Mfg", CLOUD_DATA: "Cloud & Data", COMPUTER_ELECTRONICS_MFG: "Electronics Mfg",
  CONSTRUCTION: "Construction", CONSULTING: "Consulting", DENTISTS: "Dentistry",
  DEPARTMENT_STORES: "Retail & Dept Stores", DIGITAL_CONTENT: "Digital Content", ECOMMERCE: "E-Commerce",
  ENERGY_MINING: "Energy & Mining", ENGINEERING_SERVICES: "Engineering Services",
  FOOD_BEVERAGE_MFG: "Food & Beverage", FOOTWEAR_APPAREL_MFG: "Apparel & Footwear",
  HOLDING_COMPANIES: "Holding Companies", HOSPITALS: "Hospitals", HOTELS: "Hotels & Hospitality",
  INDUSTRIAL_MFG: "Industrial Mfg", INSTRUMENTS_MFG: "Instruments Mfg", INSURANCE: "Insurance",
  INTERNET_PLATFORMS: "Internet Platforms", INVESTMENT_SECURITIES: "Investment & Securities",
  IT_SERVICES: "IT Services", K12_SCHOOLS: "K-12 Schools", LEGAL: "Legal",
  MEDIA_ENTERTAINMENT: "Media & Entertainment", MEDICAL_DEVICES_MFG: "Medical Devices",
  OTHER: "Other", OTHER_EDUCATION: "Education (Other)", OTHER_HOSPITALITY: "Hospitality (Other)",
  OTHER_MFG: "Manufacturing (Other)", OTHER_PROFESSIONAL: "Professional (Other)",
  OTHER_RETAIL: "Retail (Other)", OTHER_SERVICES: "Services (Other)", OUTPATIENT_HEALTH: "Outpatient Health",
  PHARMA_MFG: "Pharma", PHYSICIANS: "Physicians & Clinics", PUBLIC_ADMIN: "Public Administration",
  REAL_ESTATE: "Real Estate", RESIDENTIAL_CARE: "Residential Care", RESTAURANTS: "Restaurants",
  SCIENTIFIC_RD: "Scientific R&D", SEMICONDUCTOR_MFG: "Semiconductor", SOCIAL_SERVICES: "Social Services",
  SOFTWARE_PRODUCTS: "Software", SPECIALTY_HOSPITALS: "Specialty Hospitals", STAFFING: "Staffing",
    ...
    <div style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: "#f8f7f4", minHeight: "100vh", color: "#1a1a1a" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .panel-btn { display:block; width:100%; padding:11px; background:#0f1117; color:#fff; border:none; border-radius:10px; font-size:13px; font-weight:600; font-family:inherit; cursor:pointer; text-align:center; margin-top:14px; }
      ...
      <div className="section" style={{ paddingTop: 28 }}>
        <div className="search-row">
          ...
          <select className="filter-select" value={stateFilter} onChange={(e) => setStateFilter(e.target.value)} style={{ minWidth: 110 }}>
        ...
        <div className="chip-row">
          ...
          {(["INCREASING", "NEW"] as const).map((t) => (
            <span key={t} className={`chip ${trendFilter === t ? "chip-active" : ""}`} onClick={() => setTrendFilter(trendFilter === t ? "" : t)}>
              {TREND_CONFIG[t].label}
            </span>
          ))}
          {POPULAR_SECTORS.slice(0, 5).map((s) => (
            <span key={s} className={`chip ${sector === s ? "chip-active" : ""}`} onClick={() => setSector(sector === s ? "" : s)}>
              {SECTOR_LABELS[s]}
            </span>
          ))}
        </div>
      ...
      <div className="table-wrap">
        {loading ? (
          <table className="co-table">
            <thead><tr>{["#","Company","LCA Filings","Avg Salary","Trend","Green Card","Top States"].map(h => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>{Array.from({ length: 12 }).map((_, i) => <tr key={i}><td><div className="skeleton" style={{ width:20,height:14 }} /></td><td><div className="skeleton" style={{ width:180,height:14,marginBottom:6 }} /><div className="skeleton" style={{ width:100,height:11 }} /></td><td><div className="skeleton" style={{ width:60,height:14 }} /></td><td><div className="skeleton" style={{ width:60,height:14 }} /></td><td><div className="skeleton" style={{ width:60,height:22,borderRadius:20 }} /></td><td><div className="skeleton" style={{ width:50,height:14 }} /></td><td><div className="skeleton" style={{ width:80,height:20 }} /></td></tr>)}</tbody>
          </table>
        ) : companies.length === 0 ? (
